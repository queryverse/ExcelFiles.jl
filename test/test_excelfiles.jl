...
@testitem "ExcelFiles xls loading" begin
    using ExcelReaders
    using TableTraitsUtils
    using Dates
    using DataValues
    ...
    # Loading the legacy xls variant of the test data must produce the same
    # table as the xlsx variant (which has one additional column).
    filename_xls = normpath(dirname(pathof(ExcelReaders)), "..", "test", "TestData.xls")

    df = DataFrame(load(filename_xls, "Sheet1!C3:N7"))
    @test size(df) == (4, 12)
    @test df[!, 1] == [1., 1.5, 2., 2.5]
    @test df[!, 2] == ["A", "BB", "CCC", "DDDD"]
    @test df[!, 3] == [true, false, false, true]
    @test df[!, 9] == [DateTime(2015, 3, 3), DateTime(2015, 2, 4, 10, 14), DateTime(1988, 4, 9), Dates.Time(15, 2, 0)]
    @test eltype(df[!, 11]) == ExcelReaders.ExcelErrorCell

    df2 = DataFrame(load(filename_xls, "Sheet1"))
    @test size(df2) == (4, 13)
