@testitem "ExcelFiles" begin
    ...
    for filename in [joinpath(@__DIR__, "TestData.xls"), joinpath(@__DIR__, "TestData.xlsx")]
        ...
        mktempdir() do path
            file = save(joinpath(path, "file.xlsx"), input)
            output = load(joinpath(path, "file.xlsx"), "Sheet1") |> DataFrame
            @test input == output
        end
        ...
        input = (Day = ["Nov. 27","Nov. 28","Nov. 29"], Highest = [78,79,75]) |> DataFrame
        mktempdir() do path
            file = save(joinpath(path, "file.xlsx"), input, sheetname="SheetName")
            output = load(joinpath(path, "file.xlsx"), "SheetName") |> DataFrame
        end
